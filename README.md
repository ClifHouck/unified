...
There's also [complete autogenerated CLI command docs](/blob/main/docs/cmd/unified.md) available.
...
To instantiate a client you should call [`client.NewClient`](/blob/main/client/client.go):
...
Access to actual API client calls is mediated through [NetworkV1](/blob/main/types/network.go)
and [ProtectV1](/blob/main/types/protect.go) interfaces. Like so:
...
[doorbell.go](/blob/main/examples/doorbell/doorbell.go)
